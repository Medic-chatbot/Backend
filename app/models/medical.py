...
class Disease(Base, UUIDMixin, TimestampMixin):
    ...
    symptom_analyses = relationship(
        "SymptomAnalysis", back_populates="predicted_disease"
    )
...
class DiseaseSymptom(Base, UUIDMixin, TimestampMixin):
    ...
    disease_id = Column(
        UUID(as_uuid=True), ForeignKey("diseases.id"), nullable=False, index=True
    )
    symptom_id = Column(
        UUID(as_uuid=True), ForeignKey("symptoms.id"), nullable=False, index=True
    )
...
class MedicalEquipmentSubcategory(Base, UUIDMixin, TimestampMixin):
    ...
    category_id = Column(
        UUID(as_uuid=True),
        ForeignKey("medical_equipment_categories.id"),
        nullable=False,
        index=True,
    )
...
class EquipmentDisease(Base, UUIDMixin, TimestampMixin):
    ...
    equipment_subcategory_id = Column(
        UUID(as_uuid=True),
        ForeignKey("medical_equipment_subcategories.id"),
        nullable=False,
        index=True,
    )
    disease_id = Column(
