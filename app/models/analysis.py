...
from sqlalchemy import JSON, Column, Float, ForeignKey, Text
...
class SymptomAnalysis(Base, UUIDMixin, TimestampMixin):
    ...
    analysis_symptoms = relationship(
        "AnalysisSymptom", back_populates="symptom_analysis"
    )
    hospital_recommendations = relationship(
        "HospitalRecommendation", back_populates="symptom_analysis"
    )
...
class AnalysisSymptom(Base, UUIDMixin, TimestampMixin):
    ...
    symptom_analysis_id = Column(
        UUID(as_uuid=True),
        ForeignKey("symptom_analysis.id"),
        nullable=False,
        index=True,
    )
    symptom_id = Column(
        UUID(as_uuid=True), ForeignKey("symptoms.id"), nullable=False, index=True
    )
    confidence_score = Column(Float, default=1.0, nullable=False)

    # 관계 설정
    symptom_analysis = relationship(
        "SymptomAnalysis", back_populates="analysis_symptoms"
    )
...
class HospitalRecommendation(Base, UUIDMixin, TimestampMixin):
    ...
    # 관계 설정
    symptom_analysis = relationship(
        "SymptomAnalysis", back_populates="hospital_recommendations"
    )
